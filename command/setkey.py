...
class SetKeyHandler(Command):
    def execute(self, cli, socket, cmdline):
        if len(cmdline) != 3:
            cli.invalid_setkey_args()
            return
        ...
        if index_key >= 4:
            ...
            self.set_key(cli, socket, index_key, bytes_data)

    def set_key(self, cli, socket, index_key: str, bytes_data: str):
        valid_data = self.is_valid_hexa(cli, bytes_data)
        if valid_data:
            cli.command_attempt()
            socket.set_key(index_key, bytes_data)
