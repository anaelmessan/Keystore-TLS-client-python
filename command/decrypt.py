...
class DecryptHandler(Command):
    def execute(self, cli, socket, cmdline):
        ...
        if index_key >= 4:
            ...
            self.decrypt(cli, socket, index_key, bytes_data)

    def decrypt(self, cli, socket, index_key: str, bytes_data: str):
        valid_data = self.is_valid_hexa(cli, bytes_data)
        if valid_data:
            cli.command_attempt()
            socket.decrypt(index_key, bytes_data)
